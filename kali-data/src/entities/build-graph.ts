import type {
  EntityGraph, Tenant, Person, Organization, EventEntity, Donation, Campaign,
  Grant, Document, EmailEntity, CalendarEvent, ZoomMeeting, PowerAutomateFlow,
  PowerBIDashboard, QBTransaction, KnowBe4Result, SolanaTx,
} from "./types.ts";
import { makeFaker, makeRng, pick, pickN, chance, intBetween, powerLaw, dateBetween, driftName } from "../lib/random.ts";
...
export interface SizeConfig {
  ...
  emails: number;
  zoomMeetings: number;
  qbTransactionsPerMonth: number;
  solanaTxs: number;
  duplicateDonorRate: number;
...
export function buildGraph(size: Size): EntityGraph {
  ...
  const programs = PROGRAMS_TEMPLATE.map(p => ({
    ...
    name: p.name,
    description: p.desc,
    budgetAnnual: p.budget,
    startedYear: p.started,
  }));
  ...
  for (let i = 0; i < cfg.staff; i++) {
    ...
    const p = makePerson({
      ...
      employer: tenant.id,
      jobTitle: role,
      email: "", // filled below as org email
    });
  ...
  // donors w/ realistic distribution
  const donorPeople: Person[] = [];
  for (let i = 0; i < cfg.donors; i++) {
    // 5% major, 20% mid, 75% grassroots — power-law on giving
    const segmentRoll = rng();
    let segment: Person["donorSegment"];
    let lifetime: number;
  ...
  // duplicate donor mess — same person, drifted name spelling, possibly different email
  const duplicates = Math.floor(donorPeople.length * cfg.duplicateDonorRate);
  for (let i = 0; i < duplicates; i++) {
    const original = pick(rng, donorPeople);
    const dup = makePerson({
      isDonor: true,
      donorSegment: "grassroots",
      lifetimeGiving: Math.round((original.lifetimeGiving ?? 100) * (0.1 + rng() * 0.4)),
      firstName: driftName(rng, original.firstName),
      lastName: original.lastName,
      // sometimes same email, sometimes a typo, sometimes blank
      email: chance(rng, 0.3) ? original.email : (chance(rng, 0.5) ? original.email.replace(".com", ".con") : ""),
      address: original.address, // same address — entity resolver should catch this
    });
    people.push(dup);
  }
  ...
  for (let yr = currentYear - 2; yr <= currentYear; yr++) {
    campaigns.push({
      ...
      startDate: `${yr}-01-01`,
      endDate: `${yr}-12-31`,
      goal: 150000 + intBetween(rng, 0, 100000),
    });
    campaigns.push({
      id: id("camp"),
      name: `Year-End Giving ${yr}`,
      type: "year_end",
      startDate: `${yr}-11-01`,
      endDate: `${yr}-12-31`,
      goal: 80000 + intBetween(rng, 0, 60000),
    });
    campaigns.push({
      id: id("camp"),
      name: `Giving Tuesday ${yr}`,
      type: "giving_tuesday",
      startDate: `${yr}-11-25`,
      endDate: `${yr}-12-05`,
      goal: 25000 + intBetween(rng, 0, 25000),
    });
  }
  while (campaigns.length < cfg.campaigns) {
    ...
    campaigns.push({
      id: id("camp"),
      name: `${prog.name} Campaign ${yr}`,
      type: "program_specific",
      startDate: `${yr}-${String(intBetween(rng, 1, 9)).padStart(2, "0")}-01`,
      endDate: `${yr}-${String(intBetween(rng, 10, 12)).padStart(2, "0")}-31`,
      goal: 30000 + intBetween(rng, 0, 70000),
      programId: prog.id,
    });
  }
  ...
  for (let i = 0; i < cfg.events; i++) {
    ...
    const attendeeIds = pickN(rng, attendeePool, attendeeCount).map(p => p.id);
    events.push({
      id: id("evt"),
      ...
      type: evType,
      date: `${yr}-${String(month).padStart(2, "0")}-${String(day).padStart(2, "0")}`,
  ...
  for (const donor of donorPeople) {
    ...
    for (let i = 0; i < giftCount; i++) {
      const amount = donor.donorSegment === "major" ? Math.round(powerLaw(rng, 1000, 100000, 2.0))
                   : donor.donorSegment === "mid" ? Math.round(powerLaw(rng, 100, 5000, 2.5))
                   : Math.round(powerLaw(rng, 10, 250, 2.5));
      ...
      donations.push({
        ...
        isMatched,
        matchedAmount: isMatched ? Math.min(amount, employer?.matchingGiftCap ?? amount) : undefined,
  ...
  for (let i = 0; i < cfg.grants; i++) {
    ...
    grants.push({
      id: id("grant"),
      funderId: funder.id,
      programId: chance(rng, 0.7) ? pick(rng, programs).id : undefined,
      status,
      amount,
  ...
  for (let i = 0; i < cfg.documents; i++) {
    ...
    const title = docType === "board_minutes" ? `Board Meeting Minutes — ${created.toISOString().slice(0, 10)}`
               : docType === "program_report" ? `${program?.name ?? "Program"} Quarterly Report — ${created.toISOString().slice(0, 7)}`
               : docType === "grant_application" ? `Grant Application — ${pick(rng, foundations).name}`
               : docType === "financial_statement" ? `Financial Statement — ${created.toISOString().slice(0, 7)}`
               : docType === "policy" ? `Policy: ${faker.lorem.words(3)}`
               : docType === "hr_record" ? `HR — ${pick(rng, staffPeople).firstName} ${pick(rng, staffPeople).lastName}`
               : docType === "communication_plan" ? `Communication Plan — ${faker.lorem.words(2)}`
               : `Annual Report — FY${created.getFullYear()}`;
    documents.push({
      id: id("doc"),
      title,
      type: docType,
  ...
  for (let i = 0; i < cfg.emails; i++) {
    ...
    const toCount = intBetween(rng, 1, 5);
    const recipients = pickN(rng, people, toCount).map(p => p.id);
    ...
    emails.push({
      ...
      toIds: recipients,
      subject: pick(rng, [
        "Re: Board prep next week",
        "Donor stewardship update — Q3",
        `Grant deadline reminder: ${pick(rng, foundations).name}`,
        "Quick question about the gala manifest",
        "FY budget revisions",
        "Year-end appeal copy review",
        "Volunteer schedule for Saturday",
        "Re: Major gift prospects",
        `${pick(rng, programs).name} program update`,
        "Vendor invoice — please approve",
        "Salesforce report",
        "Quarterly impact metrics",
        faker.lorem.sentence({ min: 3, max: 8 }).slice(0, 60),
      ]),
      date: dt.toISOString(),
  ...
  for (const staff of staffPeople) {
    ...
    for (let i = 0; i < meetingCount; i++) {
      ...
      const attendees = pickN(rng, [...staffPeople, ...people.filter(p => p.isBoard)], intBetween(rng, 1, 6)).map(p => p.id);
      calendarEvents.push({
        id: id("cal"),
        ownerId: staff.id,
  ...
  for (let i = 0; i < cfg.zoomMeetings; i++) {
    ...
    const attendees = pickN(rng, [...staffPeople, ...people.filter(p => p.isBoard || p.isDonor)], intBetween(rng, 2, 30)).map(p => p.id);
    ...
    zoomMeetings.push({
      ...
      attendeeIds: attendees,
      hasRecording: hasRec,
      hasTranscript: hasRec && chance(rng, 0.7),
  ...
  for (let i = 0; i < Math.min(cfg.events / 3, FLOW_TEMPLATES.length); i++) {
    ...
    for (let j = 0; j < intBetween(rng, 30, 220); j++) {
      runs.push({
        ...
        status: chance(rng, 0.92) ? "success" as const : "failure" as const,
  ...
  // mirror donations as income transactions
  for (const d of donations) {
    qbTransactions.push({
      id: id("qb"),
      date: d.date,
      type: "income",
      category: "Contributed Income",
      amount: d.amount,
      account: pick(rng, ["Operating Checking", "Restricted Checking"]),
      programId: d.programDesignation,
      donationId: d.id,
      memo: `Gift from donor ${d.donorId}${d.campaignId ? ` (${campaigns.find(c => c.id === d.campaignId)?.name})` : ""}`,
    });
  }
  // generate generic operating expenses
  const monthsBack = 18;
  ...
  const knowBe4Results: KnowBe4Result[] = staffPeople.map(s => {
    ...
    for (let i = 0; i < phishCount; i++) {
      const result = rng() < 0.85 ? "passed" as const : (rng() < 0.6 ? "failed_clicked" as const : "failed_credentials" as const);
      if (result !== "passed") failures++;
      phishingTests.push({
        date: dateBetween(rng, new Date(Date.now() - 365 * 86400000), new Date()).toISOString().slice(0, 10),
        result,
      });
    }
    ...
    return {
      userId: s.id,
      riskScore,
      trainingCompletionPct: intBetween(rng, 60, 100),
      phishingTests,
      flagged,
    };
  });
  ...
  for (let i = 0; i < cfg.solanaTxs; i++) {
    ...
    let refKind: SolanaTx["reference"]["kind"];
    let refId: string;
    let amount: number;
    if (txType === "grant_disbursement") {
      const partner = pick(rng, organizations.filter(o => o.type === "partner" || o.type === "foundation"));
      recipient = partner;
    ...
    solanaTxs.push({
      ...
      feeLamports: intBetween(rng, 5000, 12000),
      status: "confirmed",
    });
  }
