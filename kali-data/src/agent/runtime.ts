// Kali agent runtime. Single Claude Sonnet 4.6 orchestrator with all 60+ tools
// available. Implements the full tool-use loop with streaming + audit logging.
//
// Usage:
//   ANTHROPIC_API_KEY=... bun run src/agent/runtime.ts medium "find lapsed donors with matching gift programs"
//   ANTHROPIC_API_KEY=... bun run src/agent/runtime.ts medium  # interactive mode
  ...
  for (let iter = 0; iter < MAX_ITERATIONS; iter++) {
    ...
    const results = await Promise.all(toolUseBlocks.map(async (block) => {
      ...
      try {
        ...
        toolCalls.push({ name: block.name, input: block.input, result: out, durationMs: dur });
...
async function main() {
  ...
  if (!query) { console.error('usage: bun run src/agent/runtime.ts <size> "<query>"'); process.exit(1); }
