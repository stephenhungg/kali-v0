...
// against the seeded connector JSON. ~25 tools across 11 connectors —
...
export async function loadConnectorData(size: ToolContext["size"]): Promise<ToolContext["data"]> {
  const dir = path.join(process.cwd(), "data", "seed", size);
  ...
  for (const t of tools) {
    data[t] = JSON.parse(await readFile(path.join(dir, `${t}.json`), "utf8"));
  }
  data._graph = JSON.parse(await readFile(path.join(dir, "_entity_graph.json"), "utf8"));
  return data;
}
...
export const TOOLS: ToolDef[] = [
  {
    ...
    description: "Search donors in Bloomerang. Filter by segment (major|mid|grassroots|lapsed|prospect), minimum lifetime giving, days since last gift, and matching-gift eligibility (via employer). Returns up to 50 donors.",
    input_schema: { type: "object", properties: { segment: { type: "string", enum: ["major", "mid", "grassroots", "lapsed", "prospect"] }, minLifetimeGiving: num, maxDaysSinceLastGift: intg, minDaysSinceLastGift: intg, employerHasMatchingGifts: bool } },
    fn: async (input: any, ctx) => {
      ...
      return { count: filtered.length, donors: filtered.slice(0, 50).map(c => ({ kali_entity_id: c.kali_entity_id, name: `${c.firstName} ${c.lastName}`, email: c.primaryEmail.value, segment: c.donorSegment, lifetimeGiving: c.lifetimeGiving, lastGiftDate: c.lastGiftDate, engagementLevel: c.engagement.level, employer: c.employer })) };
    },
  },
  {
  ...
  },
  {
    ...
    input_schema: { type: "object", properties: { donorKaliId: str, startDate: str, endDate: str, minAmount: num, maxAmount: num, limit: intg } },
    fn: async (input: any, ctx) => {
      ...
      return { count: txs.length, totalAmount: txs.reduce((s, t) => s + t.amount, 0), transactions: txs.slice(0, input.limit ?? 100) };
    },
  },
  {
    name: "salesforce_get_account",
    description: "Get a Salesforce Account (organization) by kali_entity_id.",
    input_schema: { type: "object", properties: { kali_entity_id: str }, required: ["kali_entity_id"] },
  ...
  },
  {
    ...
    input_schema: { type: "object", properties: { type: str, hasMatchingGifts: bool, fundingFocus: str } },
    fn: async (input: any, ctx) => {
      let accts: any[] = ctx.data.salesforce.accounts;
      if (input.type) accts = accts.filter(a => a.Type.toLowerCase() === input.type.toLowerCase());
  ...
  },
  {
    ...
    description: "Search Salesforce contacts. Filter by isBoard, isMajorDonor, employer kali_entity_id.",
    input_schema: { type: "object", properties: { isBoard: bool, isMajorDonor: bool, employerKaliId: str, minLifetimeGiving: num } },
    fn: async (input: any, ctx) => {
      ...
      return { count: contacts.length, contacts: contacts.slice(0, 50).map(c => ({ kali_entity_id: c.kali_entity_id, name: `${c.FirstName} ${c.LastName}`, email: c.Email, title: c.Title, isBoard: c.npsp__Board_Member__c, isMajorDonor: c.npsp__Major_Donor__c, lifetimeGiving: c.npsp__LifetimeGivingTotal__c, employerKaliId: ctx.data.salesforce.accounts.find((a: any) => a.Id === c.AccountId)?.kali_entity_id })) };
    },
  },
  {
    name: "zoom_search_meetings",
    description: "Search Zoom meetings by topic substring or date range. Use to find which donors attended events (e.g. galas).",
    input_schema: { type: "object", properties: { topicContains: str, startDate: str, endDate: str } },
    fn: async (input: any, ctx) => {
      ...
      return { count: meetings.length, meetings: meetings.slice(0, 30).map(m => ({ kali_entity_id: m.kali_entity_id, topic: m.topic, startTime: m.startTime, durationMin: m.duration, attendeeCount: m.participants.length, attendeeKaliIds: m.participants.map((p: any) => p.userId), hasTranscript: !!m.transcript })) };
    },
  },
  {
  ...
  },
  {
    ...
    description: "Search grants. Filter by status, days until deadline, funder, program, fit score.",
    input_schema: { type: "object", properties: { status: str, maxDaysToDeadline: intg, funderKaliId: str, programKaliId: str, minFitScore: num } },
    fn: async (input: any, ctx) => {
  ...
  },
  {
    ...
    description: "Get a funder's profile from Instrumentl by kali_entity_id.",
    input_schema: { type: "object", properties: { kali_entity_id: str }, required: ["kali_entity_id"] },
  ...
  { name: "quickbooks_pnl_summary", description: "Get trailing-12-months P&L summary.", input_schema: { type: "object", properties: {} }, fn: async (_input: any, ctx) => ctx.data.quickbooks.pnl },
  { name: "quickbooks_program_budgets", description: "Get budget vs actual for every program.", input_schema: { type: "object", properties: {} }, fn: async (_input: any, ctx) => ctx.data.quickbooks.budgetVsActual },
  {
    name: "sharepoint_search_documents",
    description: "Search SharePoint documents by query string (matches title, body, tags).",
    input_schema: { type: "object", properties: { query: str, docType: str, programKaliId: str, grantKaliId: str, modifiedAfter: str } },
    fn: async (input: any, ctx) => {
      ...
      return { count: files.length, files: files.slice(0, 20).map((f: any) => ({ kali_entity_id: f.kali_entity_id, name: f.name, type: f.type, modifiedDateTime: f.lastModifiedDateTime, siteId: f.siteId, snippet: f.body.slice(0, 300), tags: f.tags })) };
    },
  },
  { name: "sharepoint_get_document", description: "Get full SharePoint document body by kali_entity_id.", input_schema: { type: "object", properties: { kali_entity_id: str }, required: ["kali_entity_id"] }, fn: async (input: any, ctx) => ctx.data.sharepoint.files.find((f: any) => f.kali_entity_id === input.kali_entity_id) ?? null },
  {
    name: "m365_search_emails",
    description: "Search M365 emails by subject substring, recipient, or date range.",
    input_schema: { type: "object", properties: { subjectContains: str, recipientKaliId: str, fromKaliId: str, startDate: str, endDate: str } },
    fn: async (input: any, ctx) => {
  ...
  },
  {
    ...
    description: "Get the most recent email sent to a specific kali_entity_id.",
  ...
  },
  {
    ...
    description: "Get a Power BI dashboard's metrics by name (substring match).",
    input_schema: { type: "object", properties: { nameContains: str }, required: ["nameContains"] },
    fn: async (input: any, ctx) => ctx.data.powerBI.dashboards.find((d: any) => d.displayName.toLowerCase().includes(input.nameContains.toLowerCase())) ?? null,
  },
  { name: "knowbe4_org_posture", description: "Get aggregate org cybersecurity posture.", input_schema: { type: "object", properties: {} }, fn: async (_input: any, ctx) => ctx.data.knowbe4.orgPosture },
  { name: "knowbe4_get_user", description: "Get a staff member's cybersecurity profile by kali_entity_id.", input_schema: { type: "object", properties: { kali_entity_id: str }, required: ["kali_entity_id"] }, fn: async (input: any, ctx) => ctx.data.knowbe4.userResults.find((u: any) => u.kali_entity_id === input.kali_entity_id) ?? null },
  { name: "solana_get_treasury", description: "Get the nonprofit's onchain treasury wallet (USDC + SOL balances, devnet).", input_schema: { type: "object", properties: {} }, fn: async (_input: any, ctx) => ctx.data.solana.treasury },
  {
    name: "solana_recent_disbursements",
    description: "List recent onchain disbursements with explorer URLs.",
    input_schema: { type: "object", properties: { type: str, startDate: str, limit: intg } },
    fn: async (input: any, ctx) => {
  ...
  },
  {
    ...
    description: "Execute a batch onchain payout. SIMULATED in v1 — generates valid-looking signatures and explorer URLs without hitting devnet.",
    input_schema: { type: "object", properties: { payouts: { type: "array", items: { type: "object", properties: { recipientKaliId: str, amountUsdc: num, type: { type: "string", enum: ["grant_disbursement", "vendor_payment", "board_stipend", "donor_refund"] }, referenceKaliId: str, memo: str }, required: ["recipientKaliId", "amountUsdc", "type"] } } }, required: ["payouts"] },
    fn: async (input: any, _ctx) => {
      const sigs = input.payouts.map((_p: any) => Array.from({ length: 88 }, () => Math.random().toString(36)[2]).join(""));
      return { success: true, executedCount: input.payouts.length, totalUsdc: input.payouts.reduce((s: number, p: any) => s + p.amountUsdc, 0), feesUsd: input.payouts.length * 0.0001, signatures: sigs, explorerUrls: sigs.map((s: string) => `https://explorer.solana.com/tx/${s}?cluster=devnet`), confirmedInMs: 412 };
    },
  },
  { name: "list_programs", description: "List the nonprofit's programs.", input_schema: { type: "object", properties: {} }, fn: async (_input: any, ctx) => ctx.data._graph.tenant.programs },
  { name: "list_board_members", description: "List all board members.", input_schema: { type: "object", properties: {} }, fn: async (_input: any, ctx) => ctx.data._graph.people.filter((p: any) => p.isBoard).map((p: any) => ({ kali_entity_id: p.id, name: `${p.firstName} ${p.lastName}`, title: p.jobTitle, employer: p.employer })) },
];
