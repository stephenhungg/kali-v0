...
// resemble what each SaaS would return from its own API.
import type { EntityGraph } from "./types.ts";
...
function projectBloomerang(g: EntityGraph) {
  const constituents = g.people.filter(p => p.isDonor || p.isProspect).map(p => ({
    ...
    firstGiftDate: p.firstGiftDate ?? null, lastGiftDate: p.lastGiftDate ?? null,
  ...
  const transactions = g.donations.map(d => ({
    ...
    kali_entity_id: d.id, constituentId: d.donorId.replace("ppl_", "bl_"),
    amount: d.amount, date: d.date, paymentMethod: d.paymentMethod,
    isMatched: d.isMatched ?? false, matchedAmount: d.matchedAmount ?? 0,
    campaignId: d.campaignId, appealId: d.eventId, fundDesignation: d.programDesignation,
    acknowledged: d.acknowledged ?? false, thankYouSentDate: d.thankYouSentDate ?? null,
  }));
  const onlineForms = [
...
function projectSalesforce(g: EntityGraph) {
  const accounts = g.organizations.map(o => ({
    Id: `001${o.id.slice(-15).padStart(15, "0")}`, kali_entity_id: o.id, Name: o.name,
  ...
  const contacts = g.people.filter(p => p.isDonor || p.isBoard || p.isProspect).map(p => ({
    Id: `003${p.id.slice(-15).padStart(15, "0")}`, kali_entity_id: p.id,
    FirstName: p.firstName, LastName: p.lastName,
    ...
    npsp__Board_Member__c: p.isBoard, npsp__Major_Donor__c: p.donorSegment === "major",
  ...
  const opportunities = g.donations.map(d => ({
    Id: `006${d.id.slice(-15).padStart(15, "0")}`, kali_entity_id: d.id,
    ...
    Amount: d.amount, CloseDate: d.date, StageName: "Posted",
  ...
  const campaigns = g.campaigns.map(c => ({
    Id: `701${c.id.slice(-15).padStart(15, "0")}`, kali_entity_id: c.id,
    Name: c.name, Type: c.type, StartDate: c.startDate, EndDate: c.endDate,
    ExpectedRevenue: c.goal, ActualCost: 0,
  }));
...
function projectSharepoint(g: EntityGraph) {
  ...
  const files = g.documents.map(d => ({
    id: d.id.replace("doc_", "sp_"), kali_entity_id: d.id,
    name: d.title + (d.type === "annual_report" ? ".pdf" : ".docx"), type: d.type,
    ...
    createdBy: d.authorId, createdDateTime: d.createdDate, lastModifiedDateTime: d.modifiedDate,
    sizeBytes: d.sizeKb * 1024, tags: d.tags, body: d.body,
    sharingLinks: d.externalSharing ?? [], relatedGrant: d.grantId, relatedProgram: d.programId,
  }));
  return { sites, files };
}
...
function projectM365(g: EntityGraph) {
  ...
  const users = staff.map(s => ({
    id: s.id.replace("ppl_", "m365_"), kali_entity_id: s.id,
    userPrincipalName: s.email, displayName: `${s.firstName} ${s.lastName}`,
  ...
  const messages = g.emails.map(e => ({
    id: e.id.replace("eml_", "m365_msg_"), kali_entity_id: e.id, conversationId: e.threadId,
    ...
    subject: e.subject, bodyPreview: e.snippet,
    receivedDateTime: e.date, hasAttachments: e.hasAttachment ?? false, importance: "normal",
  }));
  const calendars = g.calendarEvents.map(c => ({
    id: c.id.replace("cal_", "m365_cal_"), kali_entity_id: c.id,
...
function projectPowerAutomate(g: EntityGraph) {
  return {
    flows: g.powerAutomateFlows.map(f => ({
      flowId: f.id.replace("flow_", "pa_"), kali_entity_id: f.id,
      displayName: f.name, description: f.description, trigger: f.trigger,
      state: f.active ? "Started" : "Stopped", createdTime: f.createdDate,
      runs: {
...
function projectPowerBI(g: EntityGraph) {
  return {
    dashboards: g.powerBIDashboards.map(d => ({
      id: d.id.replace("dash_", "pbi_"), kali_entity_id: d.id, displayName: d.name,
      embedUrl: `https://app.powerbi.com/dashboardEmbed?dashboardId=${d.id}`,
      tiles: d.metrics.map((m, i) => ({
        tileId: `tile_${d.id}_${i}`, title: m.name,
        currentValue: m.value, previousValue: Math.round(m.value / (1 + m.trendPct)), trendPct: m.trendPct,
      })),
    })),
  };
}
...
function projectQuickbooks(g: EntityGraph) {
  ...
  const transactions = g.qbTransactions.map(t => ({
    id: t.id.replace("qb_", "qb_tx_"), kali_entity_id: t.id,
    txnDate: t.date,
    txnType: t.type === "income" ? "Deposit" : t.type === "expense" ? "Bill" : "Transfer",
    accountRef: t.account, amount: t.amount, category: t.category,
    classRef: t.programId ? g.tenant.programs.find(p => p.id === t.programId)?.name : null,
    vendorRef: t.vendorId, memo: t.memo,
  }));
...
function projectInstrumentl(g: EntityGraph) {
  const grants = g.grants.map(gr => {
    ...
    return {
      grantId: gr.id.replace("grant_", "inst_"), kali_entity_id: gr.id,
      title: `${funder?.name ?? "Funder"} — ${program?.name ?? "General Operating"} Grant`,
      funderName: funder?.name ?? "Unknown Funder", funderId: gr.funderId,
      amountRange: { min: Math.round(gr.amount * 0.8), max: Math.round(gr.amount * 1.2) },
      requestedAmount: gr.amount, awardedAmount: gr.amountAwarded ?? null,
      status: gr.status, deadline: gr.deadline,
      submittedDate: gr.submittedDate, awardedDate: gr.awardedDate, reportDueDate: gr.reportDueDate,
      fitScore: gr.fitScore, programArea: program?.name,
      fundingFocus: funder?.fundingFocus ?? [], notes: gr.notes, relatedDocuments: gr.documents,
    };
  });
  const funders = g.organizations.filter(o => o.type === "foundation" || o.type === "government").map(f => ({
    funderId: f.id, name: f.name, type: f.type, fundingFocus: f.fundingFocus ?? [],
...
function projectKnowBe4(g: EntityGraph) {
  const userResults = g.knowBe4Results.map(r => {
    ...
    return {
      kbUserId: r.userId.replace("ppl_", "kb_"), kali_entity_id: r.userId,
      userName: u ? `${u.firstName} ${u.lastName}` : "Unknown", email: u?.email ?? null,
      ...
      riskScore: r.riskScore, trainingCompletionPercent: r.trainingCompletionPct,
      phishingTests: r.phishingTests, flagged: r.flagged ?? [],
    };
  });
  const orgPosture = {
...
function projectZoom(g: EntityGraph) {
  const meetings = g.zoomMeetings.map(z => ({
    meetingId: z.id.replace("zoom_", "zm_"), kali_entity_id: z.id, hostId: z.hostId,
    topic: z.topic, startTime: z.startTime, duration: z.durationMin, type: 2,
...
function projectSolana(g: EntityGraph) {
  return {
    ...
    transactions: g.solanaTxs.map(t => ({
      signature: t.signature, kali_entity_id: t.id, type: t.type, amountUsdc: t.amountUsdc,
      blockTime: Math.floor(new Date(t.date).getTime() / 1000),
      fromWallet: t.fromWallet, toWallet: t.toWallet, recipientId: t.recipientId,
      reference: t.reference, feeLamports: t.feeLamports, feeUsd: t.feeLamports * 0.00000003 * 165,
      status: t.status, explorerUrl: `https://explorer.solana.com/tx/${t.signature}?cluster=devnet`,
    })),
  };
}
